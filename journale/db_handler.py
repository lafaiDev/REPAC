...
class meedDBHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        request = """INSERT INTO meed_log (log_date, log_uid, model_name, res_id, pid, level, message)
    ...
    def close(self):
        ...
        for cr in self._dbname_to_cr.values():
            try:
                cr.execute("INSERT INTO meed_log (log_date, log_uid, model_name, res_id, pid, level, message) "
...
class meedDBLogger():
    ...
    def __init__(self, dbname, model_name, res_id, uid=0):
        ...
        self._logger = logging.getLogger('meed_log')

        db, pool = pooler.get_db_and_pool(dbname)
        pid = 0
        try:
            cr = db.cursor()
            cr.execute("select relname from pg_class where relname='meed_log_seq'")
            if not cr.rowcount:
                cr.execute("create sequence meed_log_seq")
            cr.execute("select nextval('meed_log_seq')")
            res = cr.fetchone()
...
logging.getLogger('meed_log').addHandler(meedDBHandler())
